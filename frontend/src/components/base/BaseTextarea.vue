<template>
  <div>
    ...
    <textarea
      ...
      class="min-h-20 w-full resize-y rounded-md border bg-surface-raised px-3 py-2 text-sm text-ink placeholder:text-ink-muted transition-colors hover:border-ink-muted disabled:cursor-not-allowed disabled:bg-surface-sunken disabled:text-ink-muted"
      :class="hasError ? 'border-danger focus-visible:outline-danger' : 'border-line focus-visible:border-primary'"
      @input="onInput"
    />
    <p v-if="maxlength" class="mt-1 text-right text-xs text-ink-muted">{{ modelValue.length }}/{{ maxlength }}</p>
